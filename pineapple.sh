...
#Fix _NET_WM_ICON
curl -s https://patch-diff.githubusercontent.com/raw/yuzu-emu/yuzu/pull/5274.diff > ./icon-fix.diff || curl -s https://gitlab.com/samantas5855/pineapple/-/raw/master/5274.diff > ./icon-fix.diff
patch -p1 < icon-fix.diff
...
#Comment out git naming stuff
sed -i -e '14,17 {s/^/#/}' CMakeModules/GenerateSCMRev.cmake
sed -i -e '/string(MD5 SHADER_CACHE_VERSION "${COMBINED}")/a set(BUILD_FULLNAME "${BUILD_FULLNAME} ")' CMakeModules/GenerateSCMRev.cmake
#Add custom CMake flags
sed -i -e '/-DGIT_EXECUTABLE="${GIT_EXECUTABLE}"/a \\t  -DBUILD_DATE="${BUILD_DATE}"' src/common/CMakeLists.txt
sed -i -e '/-DGIT_EXECUTABLE="${GIT_EXECUTABLE}"/a \\t  -DBUILD_FULLNAME="${BUILD_FULLNAME}"' src/common/CMakeLists.txt
sed -i -e '/-DGIT_EXECUTABLE="${GIT_EXECUTABLE}"/a \\t  -DGIT_BRANCH="${GIT_BRANCH}"' src/common/CMakeLists.txt
sed -i -e '/-DGIT_EXECUTABLE="${GIT_EXECUTABLE}"/a \\t  -DGIT_DESC="${GIT_DESC}"' src/common/CMakeLists.txt
sed -i -e '/-DGIT_EXECUTABLE="${GIT_EXECUTABLE}"/a \\t  -DGIT_REV="${GIT_REV}"' src/common/CMakeLists.txt
sed -i -e '/-DGIT_EXECUTABLE="${GIT_EXECUTABLE}"/a \\t  -DGIT_REF_SPEC="${GIT_REF_SPEC}"' src/common/CMakeLists.txt
